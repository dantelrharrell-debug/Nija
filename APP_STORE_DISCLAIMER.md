# App Store Disclaimer Copy

## Primary Disclaimer (Required on App Listing)

### Short Version (App Store Subtitle - Max 30 characters)
```
Automated Trading Algorithm
```

### Full Description Disclaimer
```
IMPORTANT DISCLAIMER - PLEASE READ CAREFULLY

NIJA is an automated cryptocurrency trading application that executes trades 
on your behalf using algorithmic analysis. This app operates on YOUR exchange 
accounts using YOUR API credentials.

INDEPENDENT TRADING MODEL:
NIJA does NOT copy trades between accounts. Each connected account trades 
independently using the same NIJA trading algorithm. All accounts analyze 
markets independently and make their own trading decisions. There is NO 
platform account that controls other accounts, and NO trade signal copying 
occurs between accounts.

SUBSTANTIAL RISK OF LOSS:
Trading cryptocurrencies involves substantial risk of loss. You may lose 
some or all of your invested capital. Only invest money you can afford to 
lose. Past performance does not guarantee future results.

NOT FINANCIAL ADVICE:
NIJA is a software tool, NOT a financial advisor. We do NOT provide investment 
advice, recommendations, or financial planning services. You are solely 
responsible for your trading decisions.

NO GUARANTEED RETURNS:
There are NO guaranteed profits. Any representation of potential earnings or 
returns is purely hypothetical and not a guarantee of actual performance. 
Trading results vary widely and depend on market conditions, account size, 
risk settings, and timing.

YOUR RESPONSIBILITY:
• You maintain full control of your exchange accounts
• You are responsible for all trades executed by the algorithm
• You must monitor your account regularly
• You should understand the trading strategy before use
• You must comply with all applicable laws and regulations in your jurisdiction

TECHNICAL REQUIREMENTS:
• Valid exchange API credentials (read and trade permissions)
• Stable internet connection
• Minimum account balance: $50 USD recommended
• Compatible exchange: Coinbase, Kraken, or other supported platforms

AGE RESTRICTION:
You must be 18 years or older (21+ in some jurisdictions) to use this app.

GEOGRAPHIC RESTRICTIONS:
This app may not be available or legal in all jurisdictions. Users are 
responsible for compliance with local laws. Cryptocurrency trading may be 
restricted or prohibited in your country.

BY DOWNLOADING AND USING THIS APP, YOU ACKNOWLEDGE THAT YOU HAVE READ, 
UNDERSTOOD, AND AGREE TO THESE TERMS AND ACCEPT ALL ASSOCIATED RISKS.
```

---

## In-App Disclaimers

### First Launch Screen
```
═══════════════════════════════════════════════════════════
                    WELCOME TO NIJA
═══════════════════════════════════════════════════════════

⚠️  CRITICAL INFORMATION - PLEASE READ

NIJA is an automated trading algorithm that will execute 
cryptocurrency trades on your exchange account(s).

INDEPENDENT TRADING:
• Each account trades independently using the same algorithm
• Accounts do NOT copy trades from each other
• Each account makes its own trading decisions
• No master/follower relationship exists

RISKS:
• You can lose money, potentially all invested capital
• Cryptocurrency markets are highly volatile
• Past performance ≠ future results
• No guaranteed profits

YOUR RESPONSIBILITY:
• Monitor your account regularly
• Understand the strategy before enabling
• Only invest what you can afford to lose
• Comply with your local laws

☑️  I understand and accept these risks
☑️  I am 18+ years old (21+ if required in my jurisdiction)
☑️  Cryptocurrency trading is legal in my jurisdiction
☑️  I will not hold the developers liable for trading losses

[Continue]  [Exit]
═══════════════════════════════════════════════════════════
```

### Strategy Activation Screen
```
═══════════════════════════════════════════════════════════
              ACTIVATE INDEPENDENT TRADING
═══════════════════════════════════════════════════════════

You are about to enable automated trading for:
• Account: [Account Name]
• Exchange: [Exchange Name]
• Current Balance: $[Amount]

WHAT WILL HAPPEN:
✓ NIJA will scan cryptocurrency markets every 2.5 minutes
✓ NIJA will execute BUY orders when entry criteria are met
✓ NIJA will execute SELL orders to take profits or stop losses
✓ All trades are based on NIJA's independent market analysis
✓ Position sizes will be calculated based on your account balance

INDEPENDENT TRADING:
✓ This account trades independently
✓ This account does NOT copy trades from other accounts
✓ This account uses the same NIJA algorithm as all accounts
✓ Timing and exact trades may differ from other accounts

RISKS:
⚠️  You may lose money
⚠️  Markets can move against positions
⚠️  No profits are guaranteed
⚠️  You remain responsible for all trading activity

RECOMMENDED:
□ Start with minimum capital ($50-100)
□ Monitor results for 7-30 days before adding capital
□ Keep 50%+ of funds in reserve (not allocated to trading)
□ Review trade history weekly

☑️  I understand this account will trade independently
☑️  I understand the risks and accept potential losses
☑️  I will monitor my account regularly

[Activate Trading]  [Cancel]
═══════════════════════════════════════════════════════════
```

### Daily Notification Disclaimer
```
🤖 NIJA Independent Trading Active

Your account is trading independently using the NIJA algorithm.

Today's Summary:
• Trades: [X] (independent decisions)
• P&L: $[Amount] ([+/-]%)
• Open Positions: [X]

⚠️ Remember: Past results ≠ future performance
Monitor your account regularly.

[View Details]
```

### Settings Screen Disclaimer
```
═══════════════════════════════════════════════════════════
                   TRADING SETTINGS
═══════════════════════════════════════════════════════════

⚠️  IMPORTANT: INDEPENDENT TRADING MODEL

Each connected account trades INDEPENDENTLY using the same 
NIJA trading algorithm. Accounts do NOT copy trades from 
each other.

What this means:
• Each account analyzes markets independently
• Each account makes its own trading decisions
• Timing of trades will vary between accounts
• Position sizes are based on each account's balance
• No master/follower or signal copying occurs

═══════════════════════════════════════════════════════════
```

---

## Legal Disclaimers for Terms of Service

### Independent Trading Disclosure
```
SECTION: INDEPENDENT TRADING MODEL

NIJA operates using an independent trading model. Each connected account 
trades independently using the same NIJA trading algorithm. The system does 
NOT copy, mirror, or replicate trades from one account to another.

Specifically:
a) Each account analyzes market data independently
b) Each account makes its own trading decisions based on the same algorithm
c) Each account executes trades at its own timing
d) Position sizes are calculated independently based on each account's balance
e) No account has control over or influence on other accounts
f) There is no "platform account" or "follower account" structure
g) Trade signals are NOT transmitted between accounts
h) The timing and exact trades may differ between accounts due to:
   - Network latency
   - API response times  
   - Account balance differences
   - Existing position differences
   - Natural timing variation in market scanning

Users acknowledge and agree that:
- Accounts trade independently, not by copying
- Each account is responsible for its own trading results
- Similar trades across accounts result from using the same algorithm, not copying
- No account receives preferential treatment or information
```

### Risk Disclosure
```
SECTION: RISK DISCLOSURE

SUBSTANTIAL RISK OF LOSS: Trading cryptocurrencies involves substantial risk. 
You may lose some or all of your invested capital. Factors contributing to 
loss include but are not limited to:

• Market Volatility: Cryptocurrency prices can change rapidly
• Algorithmic Risk: Automated systems can malfunction or perform poorly
• Exchange Risk: Exchanges may experience outages, hacks, or insolvency
• Execution Risk: Orders may not execute at expected prices
• Slippage: Actual execution prices may differ from expected prices
• Liquidity Risk: You may not be able to exit positions quickly
• Regulatory Risk: Laws may change affecting cryptocurrency trading

NO GUARANTEED RETURNS: There are no guaranteed profits. Any hypothetical 
performance results or backtests do not guarantee future performance. 
Past performance is not indicative of future results.

INDEPENDENT DECISION: You are solely responsible for deciding whether to use 
this software and for all trading decisions made by the algorithm on your behalf.
```

### No Financial Advice
```
SECTION: NOT FINANCIAL ADVICE

NIJA is a software tool for automated trading. It is NOT:
• A financial advisor
• An investment advisor  
• A broker or dealer
• A financial planning service
• A source of investment recommendations

We do NOT:
• Provide personalized financial advice
• Recommend specific investments
• Guarantee returns or profits
• Act as a fiduciary
• Provide tax, legal, or accounting advice

You should:
• Consult with licensed financial advisors before trading
• Understand the trading strategy before use
• Assess your own risk tolerance
• Consider your financial situation
• Comply with all applicable laws
```

---

## Apple App Store Specific Requirements

### App Review Information Notes
```
IMPORTANT FOR APP REVIEW:

1. INDEPENDENT TRADING MODEL:
   This app does NOT copy trades between accounts. Each account trades 
   independently using the same algorithm. There is no "platform account" 
   or signal distribution.

2. USER API CREDENTIALS:
   The app requires user-provided exchange API credentials. The app does 
   NOT have access to user funds. Users maintain full control via their 
   exchange accounts.

3. FINANCIAL RISK:
   Clear disclaimers are shown on first launch, strategy activation, and 
   throughout the app warning of financial risk and potential loss.

4. AGE GATE:
   The app requires users to confirm they are 18+ years old (21+ where 
   required) before proceeding.

5. GEOGRAPHIC COMPLIANCE:
   Users must confirm cryptocurrency trading is legal in their jurisdiction.
```

### Age Rating Justification
```
Appropriate Age Rating: 17+

Reason: Financial risk, potential for significant monetary loss, 
requires mature judgment and understanding of financial markets.

Content Descriptors:
- Gambling & Contests: NO
- Unrestricted Web Access: NO  
- Simulated Gambling: NO
- Financial Transactions: YES (cryptocurrency trading)
```

---

## Google Play Store Specific Requirements

### Financial Products Policy Compliance
```
GOOGLE PLAY FINANCIAL PRODUCTS COMPLIANCE:

1. Clear disclosure of risks on app listing page
2. Prominent disclaimer before any trading activity
3. No guarantees of returns in marketing materials
4. Transparent explanation of how the app works
5. User maintains control of their exchange accounts
6. No misleading claims about performance
7. Compliance with applicable financial regulations
```

### Short Description (80 characters max)
```
Automated crypto trading. Independent algorithm - no trade copying. High risk.
```

### Long Description (4000 characters max)
```
[Include full disclaimer from above, adapted for Google Play format]
```

---

## Marketing Materials Disclaimer

All marketing materials (website, social media, ads) MUST include:

```
⚠️ TRADING DISCLAIMER
Cryptocurrency trading involves substantial risk of loss. NIJA is an 
automated trading algorithm - NOT financial advice. Each account trades 
independently using the same algorithm. No trade copying occurs. Past 
performance does not guarantee future results. You may lose money.
```

---

## Website Footer
```
© 2026 NIJA Trading Systems

RISK DISCLOSURE: Trading cryptocurrencies involves substantial risk of loss 
and may not be suitable for all investors. This software is provided for 
informational and automated trading purposes only and should not be considered 
financial advice. Each account trades independently using the same algorithm - 
no trade copying or signal distribution occurs. Past performance is not 
indicative of future results. Users are solely responsible for their trading 
decisions and should only invest capital they can afford to lose. Consult with 
a licensed financial advisor before trading.

NOT FINANCIAL ADVICE: NIJA is software, not a financial advisor. We do not 
provide investment advice, recommendations, or financial planning services.

GEOGRAPHIC RESTRICTIONS: This software may not be available or legal in all 
jurisdictions. Users must comply with local laws and regulations.

18+ ONLY: You must be 18 years or older (21+ in some jurisdictions) to use 
this software.
```

---

## Email Signature / Communications
```
---
This email contains information about NIJA automated trading software.
⚠️  Trading involves risk of loss. Not financial advice. 18+ only.
Independent trading model - no trade copying occurs.
```

---

## COMPLIANCE NOTES

### For Legal Review:
- [ ] All disclaimers reviewed by legal counsel
- [ ] Complies with SEC regulations (if applicable)
- [ ] Complies with FINRA regulations (if applicable)  
- [ ] Complies with CFTC regulations (if applicable)
- [ ] Complies with local jurisdiction requirements
- [ ] Age gates properly implemented
- [ ] Risk disclosures prominently displayed
- [ ] No guarantees or misleading claims
- [ ] Independent trading model clearly communicated
- [ ] Terms of Service includes all required disclosures

### Updates Required:
If the trading model changes, ALL disclaimers must be updated within 24 hours 
and users must re-acknowledge terms.

**Last Updated**: February 1, 2026
