...
echo -e "${YELLOW}4${NC}) Apply emergency bypass (last resort)"
...
case $choice in
  ...
  4)
    echo -e "\n${BLUE}════════════════════════════════════════════════════════════${NC}"
    echo -e "${BLUE}STEP 4: Emergency Bypass (UNSAFE - Last Resort)${NC}"
    echo -e "${BLUE}════════════════════════════════════════════════════════════${NC}\n"
    
    echo -e "${RED}⚠️  WARNING: This disables safety protections!${NC}"
    echo ""
    echo -e "${GREEN}✅ Only use if:${NC}"
    echo "   • You need to trade RIGHT NOW"
    echo "   • Redis will be recovered soon"
    echo "   • You are running exactly 1 replica (numReplicas=1)"
    echo ""
    echo -e "${GREEN}✅ Instructions:${NC}"
    echo "   1. Go to NIJA service → Variables"
    echo "   2. Create new variable:"
    echo ""
    echo -e "      ${CYAN}NIJA_UNSAFE_BYPASS_DISTRIBUTED_LOCK=true${NC}"
    echo ""
    echo "   3. Restart NIJA service"
    echo ""
    echo -e "${RED}✋ CRITICAL - DISABLE AS SOON AS REDIS RECOVERS:${NC}"
    echo "   1. Set variable back to: NIJA_UNSAFE_BYPASS_DISTRIBUTED_LOCK=false"
    echo "   2. Restart NIJA service"
    echo ""
    echo -e "${RED}🚫 DO NOT:${NC}"
    echo "   • Use this with multiple replicas (data corruption!)"
    echo "   • Leave this enabled long-term (breaks single-writer guarantee)"
    echo "   • Scale up NIJA to 2+ replicas without disabling this"
    echo ""
    ;;
