#!/usr/bin/env python3
"""
Initialize NIJA with first user: Daivon Frazier

This script sets up the first user in the NIJA system with proper encryption,
permissions, and persistent storage.

Run this once to initialize the system with Daivon Frazier's account.
"""
...
def initialize_daivon_frazier():
    """Initialize system with Daivon Frazier as first user."""
    
    # Try to load existing database
    if load_user_database():
        user_mgr = get_user_manager()
        existing_user = user_mgr.get_user('daivon_frazier')
        if existing_user:
            print(f"\n✅ User 'daivon_frazier' already exists!")
    ...
    print("\n" + "="*80)
    print("INITIALIZING NIJA WITH FIRST USER: Daivon Frazier")
    print("="*80)
    ...
    print(f"\nManagement Commands:")
    print(f"  python manage_user_daivon.py status   # Check status")
    print(f"  python manage_user_daivon.py enable   # Enable trading")
    print(f"  python manage_user_daivon.py disable  # Disable trading")
    print(f"  python manage_user_daivon.py info     # Detailed info")
    print("="*80 + "\n")
