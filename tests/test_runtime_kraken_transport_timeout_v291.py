...
def test_transport_timeout_binds_actual_session_request():
    broker = _Broker()
    assert v291._ensure_transport_timeout(broker) is True

    result = broker.api.session.request("POST", "https://example.invalid/private")
...
def test_missing_direct_session_does_not_fabricate_success():
    ...
    assert v291._ensure_transport_timeout(broker) is False


def test_timeout_policy_uses_existing_configured_value():
    broker = _Broker()
    broker.API_TIMEOUT_SECONDS = 7
    assert v291._transport_timeout_s(broker) == 7.0
