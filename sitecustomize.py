...
# The shell startup handoff marks every preflight and the canonical launcher as
# deferred.  Docker normally installs an earlier .pth stub too, but the source
# module must enforce the same contract in environments where that image-layer
# guard is missing or stale.  A deferred validation interpreter must never
# start broker, readiness, or writer monitors.
if not _truthy_name("NIJA_DEFER_RUNTIME_SITE_HOOKS"):
