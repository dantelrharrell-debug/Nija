"""Kraken private transport timeout convergence v292.
...
v292 repairs only that transport binding. Every Kraken private call verifies the
...
LOGGER = logging.getLogger("nija.runtime_kraken_transport_timeout_v292")
MARKER = "20260830-kraken-transport-timeout-v292"
RELEASE_ID = "20260830-runtime-convergence-v292"
_READY_FLAG = "NIJA_RUNTIME_KRAKEN_TRANSPORT_TIMEOUT_V292_READY"
_PRIVATE_PATCH_ATTR = "_nija_kraken_transport_timeout_private_v292"
_SESSION_PATCH_ATTR = "_nija_kraken_transport_timeout_session_v292"
...
def _ensure_transport_timeout(broker: Any) -> bool:
    ...
    with _LOCK:
        ...
        @wraps(current)
        def request_v292(*args: Any, **kwargs: Any):
            kwargs.setdefault("timeout", timeout_s)
            return current(*args, **kwargs)

        setattr(request_v292, _SESSION_PATCH_ATTR, True)
        try:
            session.request = request_v292
        except Exception:
            return False

    LOGGER.critical(
        "KRAKEN_HTTP_TRANSPORT_TIMEOUT_V292_BOUND marker=%s account=%s "
...
def _patch_private_call() -> bool:
    ...
    @wraps(original)
    def private_v292(self: Any, *args: Any, **kwargs: Any):
    ...
    setattr(private_v292, _PRIVATE_PATCH_ATTR, True)
    setattr(private_v292, "__wrapped__", original)
    cls._kraken_private_call = private_v292
    return True
...
def _register_manifest() -> bool:
    try:
        ...
        if not isinstance(required, dict):
            return False
        required["runtime_kraken_transport_timeout_v292"] = _READY_FLAG
        return True
    except Exception:
        return False
...
def install() -> bool:
    ...
    log(
        "RUNTIME_KRAKEN_TRANSPORT_TIMEOUT_V292_%s marker=%s ready=%s "
