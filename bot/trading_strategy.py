...
MAX_SCAN_SECONDS = 350   # Max seconds allowed for the market scan loop itself (raised from 120 → 210 → 350)
...
class TradingStrategy:
    ...
    def __init__(self):
        ...
        if TradeQualityGate is not None:
            try:
                self.quality_gate = TradeQualityGate(min_reward_risk=1.2, require_momentum=True)
    ...
    def record_trade_with_advanced_manager(self, symbol: str, profit_usd: float, is_win: bool):
        ...
        if WIN_RATE_SCORE_SHAPER_AVAILABLE and _get_wrss_fn is not None:
            try:
                ...
                logger.debug("WinRateScoreShaper record skipped: %s", _wrss_err)
