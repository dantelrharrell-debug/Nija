        ...
        try:
            ...
            if total_active > 0:
                ...
                if active_platform_count == 0:
                    logger.warning("⚠️  PLATFORM account not connected - currently in MVP mode")
                    logger.warning("")
                    logger.warning("✅ Step 3 — Add / connect the PLATFORM Kraken account")
                    logger.warning("")
                    logger.warning("   This removes the warning and gives you:")
                    logger.warning("   • Central control")
                    logger.warning("   • Cleaner Apple review story")
                    logger.warning("   • Safer multi-user scaling")
                    logger.warning("")
                    logger.warning("   To enable PLATFORM account trading:")
                    logger.warning("   1. Set in your .env file (replace with your actual values):")
                    logger.warning("      KRAKEN_PLATFORM_API_KEY=your_actual_api_key_here")
                    logger.warning("      KRAKEN_PLATFORM_API_SECRET=your_actual_api_secret_here")
                    logger.warning("")
                    logger.warning("   2. Get API credentials at: https://www.kraken.com/u/security/api")
                    logger.warning("      (Must use Classic API key, NOT OAuth)")
                    logger.warning("")
                    logger.warning("   3. Restart the bot to activate")
                    logger.warning("")
                    logger.warning("   Right now 'user accounts only' = MVP mode, not production hierarchy.")
                    logger.warning("")
