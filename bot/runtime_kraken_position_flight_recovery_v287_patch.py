...
to reuse the same dead/pending object. That prevented platform Kraken and user
Kraken accounts from ever regaining current position proof.

v287 retires only demonstrably stale, unfinished v286 position flights before a
new bounded read is requested. It never marks a failed read successful, never
extends v285 snapshot freshness, and never fabricates positions, fills, cost
basis, balances, or exit protection. The old worker is not force-cancelled; v286
and the broker's existing read serialization remain authoritative.
"""
...
def _hard_age_s() -> float:
    try:
        value = float(os.environ.get("NIJA_KRAKEN_AUTHORITATIVE_POSITION_FLIGHT_MAX_AGE_S", "75") or 75.0)
...
def _retire_stale_flights() -> int:
    """Remove only unfinished v286 flights older than the hard-age bound."""
    try:
        v286 = _v286()
    ...
    with auth_lock:
        for key, flight in tuple(flights.items()):
            ...
            if flights.get(key) is flight:
                ...
                retired.append((int(key), age))

    for key, age in retired:
        LOGGER.warning(
            "KRAKEN_POSITION_V287_STALE_FLIGHT_RETIRED marker=%s broker_id=%s age_s=%.1f hard_age_s=%.1f old_worker_cancelled=false synthetic_success=false snapshot_freshness_unchanged=true safety_gates_bypassed=false",
            MARKER,
            key,
            age,
            _hard_age_s(),
...
__all__ = [
    ...
    "reconcile_once", "_hard_age_s", "_retire_stale_flights",
    "_patch_authoritative_positions",
]
