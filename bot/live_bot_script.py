...
def get_coinbase_client() -> Client:
    """Initialize and return Coinbase Client (cached). Raises on missing env."""
...
def _trading_loop(stop_event: threading.Event):
    """Lightweight placeholder trading loop that updates _last_health_check."""
    ...
    while not stop_event.is_set():
        iteration += 1
        try:
            # Example non-destructive health check; adapt to your SDK
        ...
        # Sleep but allow early wake via stop_event
        stop_event.wait(10)
...
def start_trading_loop(daemon: bool = True) -> None:
    """Start background trading loop; idempotent."""
...
def stop_trading_loop(timeout: float = 5.0) -> None:
    """Signal trading thread to stop and wait up to timeout seconds."""
    global _trading_thread, _thread_stop_event
...
def status_info() -> Dict[str, Any]:
    """
    Return a small status dict for health endpoints or imports.
    Example keys: client_ok (bool), trading_thread (running|stopped|none), last_health_check (iso or None), missing_env (list)
    """
    client_ok = False
    client_err = None
    try:
        # quick test: see if client can be created (cached) without raising
        get_coinbase_client()
        client_ok = True
    except Exception as e:
        client_err = str(e)
...
# Optional: allow quick manual test when module run directly
