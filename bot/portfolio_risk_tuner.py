...
class StrategyQuorum:
    ...
    def is_met(self, signals: Dict[str, float]) -> Tuple[bool, int]:
        ...
        (met: bool, count: int)
        """
        qualifying = [confidence for confidence in signals.values() if confidence >= self.min_confidence]
...
class PortfolioRiskTuner:
    ...
    def update_risk_metrics(self, var_99: float, drawdown: float) -> Tuple[bool, str]:
        """
    ...
    def update_strategy_weights(self, new_weights: Dict[str, float]) -> None:
        """
        Accept new strategy weights and clamp each to [MIN_STRATEGY_WEIGHT, MAX_STRATEGY_WEIGHT].
        ...
        with self._lock:
            self._strategy_weights = {
                strategy: max(MIN_STRATEGY_WEIGHT, min(MAX_STRATEGY_WEIGHT, weight))
    ...
    def set_allocation_caps(
        ...
        with self._lock:
            ...
            logger.info(
                ...
                max_per_asset_pct * 100,
                max_per_strategy_pct * 100,
            )
