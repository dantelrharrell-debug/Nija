...
class AlpacaBroker(BaseBroker):
    ...
    def connect(self) -> bool:
        ...
        if not enable_alpaca:
            ...
            logger.warning(
                "⚠️  Alpaca disabled (ENABLE_ALPACA=false) — skipping connection for %s",
    ...
    def _alpaca_protected_reference_price(self, symbol: str, side: str) -> float:
        """Return a current executable Alpaca quote for conservative sizing."""
        try:
            ...
            raw_price = (
                getattr(quote, "ask_price", 0.0)
                if normalized_side == "buy"
                else getattr(quote, "bid_price", 0.0)
            )
            return max(0.0, float(raw_price or 0.0))
        except Exception as exc:
            logger.warning(
                "Alpaca protected-entry quote unavailable symbol=%s side=%s: %s",
                symbol,
                side,
                exc,
            )
    ...
    @staticmethod
    def _alpaca_order_status(order: Any) -> str:
        return str(
    ...
    @staticmethod
    def _alpaca_order_class(order: Any) -> str:
        return str(
            getattr(getattr(order, "order_class", None), "value", None)
            or getattr(order, "order_class", "")
            or ""
        ).strip().lower()
    ...
    def place_market_order_with_protection(
        ...
        *,
        ...
        """Submit one Alpaca bracket and prove a real fill plus both live exit legs.

        The broker request contains the entry, take-profit and stop-loss together.
        NIJA does not report success until Alpaca confirms the parent filled and
        both expected child legs are present in acceptable live states.  If an
        unfilled parent cannot be verified promptly, NIJA requests cancellation
        and confirms a zero-fill terminal state.  Any accepted order whose final
        state cannot be proven is returned as submission-uncertain with its
        exchange order id so idempotency/reconciliation can block blind retries.
        """
        ...
        # APP_STORE_MODE is an absolute execution block.  If its state cannot
        # be loaded, fail closed instead of assuming live execution is allowed.
        try:
            try:
                from bot.app_store_mode import get_app_store_mode
            except ImportError:
                from app_store_mode import get_app_store_mode  # type: ignore[import]
            app_store_mode = get_app_store_mode()
        except Exception as exc:
            return {
                "status": "blocked",
                "error": f"APP_STORE_GUARD_UNAVAILABLE:{exc}",
                "protection_verified": False,
            }
        if app_store_mode.is_enabled():
        ...
        reference_price = self._alpaca_protected_reference_price(symbol, normalized_side)
        ...
        try:
            price_buffer_pct = float(
                os.getenv("NIJA_ALPACA_PROTECTED_ENTRY_PRICE_BUFFER_PCT", "0.01")
            )
        except (TypeError, ValueError):
            price_buffer_pct = 0.01
        price_buffer_pct = min(max(price_buffer_pct, 0.0), 0.05)
        sizing_price = reference_price * (1.0 + price_buffer_pct)

        if normalized_size_type in {"quote", "usd", "notional"}:
            share_qty = int(order_size / sizing_price)
        else:
        ...
        try:
            ...
            def _inspect(current: Any) -> Dict[str, Any]:
                current_status = self._alpaca_order_status(current)
                current_class = self._alpaca_order_class(current)
                legs = list(getattr(current, "legs", None) or [])
                take_profit_leg = None
                stop_loss_leg = None
                for leg in legs:
                    leg_status = self._alpaca_order_status(leg)
                    if (
                        take_profit_leg is None
                        and leg_status in live_leg_statuses
                    ...
                    if (
                        ...
                        stop_loss_leg = leg
                try:
                    filled_qty = float(getattr(current, "filled_qty", 0.0) or 0.0)
                except (TypeError, ValueError):
                    filled_qty = 0.0
                try:
                    filled_price = float(
                        getattr(current, "filled_avg_price", 0.0) or 0.0
                    )
                except (TypeError, ValueError):
                    filled_price = 0.0
                return {
                    "status": current_status,
                    "order_class": current_class,
                    "take_profit_leg": take_profit_leg,
                    "stop_loss_leg": stop_loss_leg,
                    "filled_qty": filled_qty,
                    "filled_price": filled_price,
                    "protection_verified": bool(
                        current_status == "filled"
                        and current_class == "bracket"
                        and filled_qty > 0.0
            ...
            last_order = order
            last_state = _inspect(order)
            # Continue through partial nested readbacks.  A single visible child
            # is not enough evidence to fail or succeed the protection contract.
            for delay_s in (0.0, 0.10, 0.20, 0.40, 0.80, 1.20):
                if delay_s:
                    time.sleep(delay_s)
                try:
                    last_order = self.api.get_order_by_id(
                        order_id,
                        GetOrderByIdRequest(nested=True),
                    )
                    last_state = _inspect(last_order)
                ...
                if last_state["protection_verified"]:
                    tp_leg = last_state["take_profit_leg"]
                    sl_leg = last_state["stop_loss_leg"]
                    return {
                        "status": "filled",
                        "order_id": str(order_id),
                        "order": last_order,
                        "filled_price": last_state["filled_price"],
                        "filled_size_usd": (
                            last_state["filled_qty"] * last_state["filled_price"]
                        ),
                        "protection_verified": True,
                        "protection_order_class": last_state["order_class"],
                        "stop_loss_price": stop_price,
                        "take_profit_price": target_price,
                        "stop_loss_order_id": str(getattr(sl_leg, "id", "") or ""),
                        "take_profit_order_id": str(getattr(tp_leg, "id", "") or ""),
                        "decision_trace_id": decision_trace_id,
                    }
                if (
                    last_state["status"] in terminal_failure_statuses
                    and last_state["filled_qty"] <= 0.0
                ):
                    return {
                        "status": last_state["status"] or "error",
                        "error": f"PROTECTED_ENTRY_{(last_state['status'] or 'FAILED').upper()}",
                        "order_id": str(order_id),
                        "protection_verified": False,
                        "submission_uncertain": False,
                    }

            # An accepted but still-unfilled parent can be canceled safely.  Do
            # not call this "canceled" until a zero-fill terminal readback proves it.
            if last_state["filled_qty"] <= 0.0 and last_state["status"] != "filled":
                try:
                    self.api.cancel_order_by_id(order_id)
                except Exception as cancel_exc:
                    logger.warning(
                        "Alpaca protected-entry cancel request failed order_id=%s: %s",
                        order_id,
                        cancel_exc,
                    )

                for delay_s in (0.0, 0.10, 0.25, 0.50):
                    if delay_s:
                        time.sleep(delay_s)
                    try:
                        last_order = self.api.get_order_by_id(
                    ...
                    if (
                        ...
                        return {
                            "status": "cancelled",
                            "error": "PROTECTED_ENTRY_CANCEL_CONFIRMED_ZERO_FILL",
                            "order_id": str(order_id),
                            "protection_verified": False,
                            "submission_uncertain": False,
                        }

            # The exchange accepted the parent and NIJA cannot prove a safe
            # terminal state.  Preserve the exchange id so the caller keeps the
            # idempotency reservation in submitted_pending and reconciles rather
            # than creating a duplicate position.
            return {
                "status": "submission_uncertain",
                "error": "PROTECTED_ENTRY_RECONCILIATION_REQUIRED",
                "order_id": str(order_id),
                "protection_verified": False,
                "submission_uncertain": True,
                "parent_status": last_state.get("status", ""),
                "filled_qty": last_state.get("filled_qty", 0.0),
                "filled_price": last_state.get("filled_price", 0.0),
                "decision_trace_id": decision_trace_id,
            }
        except Exception as exc:
            logger.error(
    ...
    def place_market_order(
        ...
        try:
            ...
            raw_status = str(getattr(order, 'status', 'submitted')).lower()
            status_map = {
                'new': 'open',
                'accepted': 'open',
