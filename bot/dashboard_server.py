...
def _get_live_strategy_context():
    """Best-effort resolver for the live TradingStrategy instance."""
    for module_name in ("nija_bot", "bot", "__main__"):
...
def _get_ai_monitoring_binding_status() -> dict:
    """Return AI monitoring binding diagnostics for operators."""
    strategy_from_context = _get_live_strategy_context()
    strategy_from_config = app.config.get("TRADING_STRATEGY")
    execution_from_config = app.config.get("EXECUTION_ENGINE")

    strategy_obj = strategy_from_config or strategy_from_context
    execution_obj = execution_from_config
    if execution_obj is None and strategy_obj is not None:
        execution_obj = getattr(strategy_obj, "execution_engine", None)

    return {
        "timestamp": datetime.now().isoformat(),
        "ai_monitoring_blueprint_registered": "ai_monitoring" in app.blueprints,
        "strategy_from_context": strategy_from_context is not None,
        "strategy_from_app_config": strategy_from_config is not None,
        "execution_engine_from_app_config": execution_from_config is not None,
        "execution_engine_attached": execution_obj is not None,
        "strategy_type": type(strategy_obj).__name__ if strategy_obj is not None else None,
        "execution_engine_type": type(execution_obj).__name__ if execution_obj is not None else None,
        "execution_has_expectancy_status": bool(
            execution_obj is not None and hasattr(execution_obj, "get_expectancy_kill_switch_status")
        ),
    }
...
@app.route('/api/ai-monitoring/binding', methods=['GET'])
def get_ai_monitoring_binding_status():
    """Self-test endpoint for AI monitoring live execution-engine binding."""
    try:
        return jsonify(_get_ai_monitoring_binding_status())
    except Exception as e:
        logger.exception("Error building AI monitoring binding status")
        return jsonify({
            "error": str(e),
            "timestamp": datetime.now().isoformat(),
        }), 500
...
if __name__ == "__main__":
    ...
    if AI_MONITORING_AVAILABLE and register_ai_monitoring:
        try:
            _strategy_ctx = _get_live_strategy_context()
            _exec_ctx = getattr(_strategy_ctx, "execution_engine", None) if _strategy_ctx is not None else None
            register_ai_monitoring(app, execution_engine=_exec_ctx, strategy=_strategy_ctx)
            logger.info(
                "✅ AI Monitoring API routes registered "
                "(/ai/regime, /ai/trade-confidence, /ai/liquidity-map, "
                "/system/brain-status, /system/execution-expectancy)"
            )
            if _exec_ctx is not None:
                logger.info("✅ AI Monitoring bound to live execution engine context")
            else:
                logger.warning("⚠️ AI Monitoring registered without live execution engine context")
        except Exception as e:
