...
def test_startup_validation_counts_okx_direct_rest_credentials(monkeypatch):
    """OKX with valid credentials should be counted as a viable broker."""
    from bot.broker_manager import AccountType, OKXBroker
...
def test_okx_adapter_does_not_import_sdk(monkeypatch):
    """OKXBrokerAdapter.connect() must never import the okx SDK."""
    ...
    monkeypatch.setattr(builtins, "__import__", guard_import)
    # No credentials → connect() returns False without touching the SDK
...
def test_startup_validation_counts_okx_as_viable_with_real_credentials(monkeypatch):
    """Startup validation counts OKX as viable when real credentials are set."""
    from bot.startup_validation import validate_exchange_configuration
    ...
    assert "OKX credentials configured and viable for direct REST trading" in info
    assert "Viable brokers: 1" in info
    # The old "OKX credentials present but ignored" message must NOT appear
    assert "OKX credentials present but ignored" not in info
