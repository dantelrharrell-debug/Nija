...
def test_tpe_capital_allocated_lifts_okx_min_notional_and_reaches_execute_action(monkeypatch) -> None:
    from bot import nija_core_loop as ncl
    ...
    monkeypatch.setattr(ncl, "_TPE_AVAILABLE", True)
    monkeypatch.setattr(ncl, "_PMC_AVAILABLE", False)
    ...
    entries, blocked, scored, _gates = loop._phase3_scan_and_enter(
        broker=apex.broker_client,
        snapshot=CycleSnapshot(balance=367.6, current_regime="normal", daily_pnl_usd=0.0, open_positions=0),
        symbols=["AAVE-USD"],
        available_slots=1,
        zero_signal_streak=0,
    )
    ...
    assert float(analysis["position_size"]) >= 10.0
    assert float(analysis["order_notional"]) >= 10.0
    assert float(analysis["capital_allocated"]) >= (18.38 - 1e-6)
