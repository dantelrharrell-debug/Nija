...
def _v184_module(*, valid=True, equity=249.04, reason="authenticated_tradebalance_equity"):
...
def test_same_cycle_provenance_records_exact_authenticated_equity(monkeypatch):
    Observation = namedtuple(
        "Observation", "value observed_monotonic observed_epoch sequence"
    )
    refresh = types.SimpleNamespace(
        live_brokers={"kraken": {"value": 0.0, "sequence": 7}},
        excluded_brokers={"kraken": {"reason": "zero"}},
    )
    ...
    batch = types.SimpleNamespace(
        _nija_v37_resolved={}, _nija_v37_lock=threading.Lock()
    )
