...
def test_execution_pipeline_uses_selected_broker_balance_before_capital_gate():
    ...
    assert captured["available_balance_usd"] == 25.0


def test_pipeline_order_submitter_passes_broker_specific_balance_to_pipeline():
    class _Broker:
        broker_type = "kraken"
        account_identifier = ""
    ...
    assert fake_pipeline.request is not None
    assert fake_pipeline.request.preferred_broker == "kraken"
    assert fake_pipeline.request.available_balance_usd == 30.0
