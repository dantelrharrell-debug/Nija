...
def test_emergency_stop_reject_is_soft_not_ecel_failure_or_exchange_sample():
    pipeline = _patched_pipeline(); request = SimpleNamespace(symbol="A-USD", side="buy")
    assert pipeline._on_order_rejected(request, "Execution gate pending (state_machine=EMERGENCY_STOP)") is None
    assert pipeline.original_called is False; assert pipeline.telemetry_calls == []


def test_terminal_unfilled_reject_is_soft_not_ecel_failure_or_exchange_sample():
    pipeline = _patched_pipeline(); request = SimpleNamespace(symbol="AXS-USD", side="buy")
...
def test_ack_timeout_reject_is_soft_not_exchange_sample():
    pipeline = _patched_pipeline(); request = SimpleNamespace(symbol="BTC-USD", side="buy")
    assert pipeline._on_order_rejected(request, "confirmed_order_rejected:ack_timeout_no_confirmed_fill_within_30s") is None
    assert pipeline.original_called is False; assert pipeline.telemetry_calls == []


def test_hardening_enforcement_is_local_soft_reject_not_exchange_fault():
    pipeline = _patched_pipeline(); request = SimpleNamespace(symbol="BTC-USD", side="buy")
    assert pipeline._on_order_rejected(request, "HARDENING_ENFORCEMENT") is None
    assert pipeline.original_called is False; assert pipeline.telemetry_calls == []


def test_position_cap_detail_is_local_soft_reject_not_exchange_fault():
    pipeline = _patched_pipeline(); request = SimpleNamespace(symbol="BTC-USD", side="buy")
    assert pipeline._on_order_rejected(request, "POSITION_CAP_EXCEEDED: Position cap reached: 2/1") is None
    assert pipeline.original_called is False; assert pipeline.telemetry_calls == []


def test_direct_soft_telemetry_call_is_suppressed_by_v254_boundary():
    pipeline = _patched_pipeline()
    assert pipeline._emit_execution_rejection_telemetry(symbol="BTC-USD", side="buy", reason="ack_timeout_no_confirmed_fill") is None
    assert pipeline.telemetry_calls == []


def test_direct_hardening_telemetry_is_suppressed():
    pipeline = _patched_pipeline()
    assert pipeline._emit_execution_rejection_telemetry(symbol="BTC-USD", side="buy", reason="HARDENING_ENFORCEMENT") is None
    assert pipeline.telemetry_calls == []


def test_unknown_direct_telemetry_still_reaches_original_boundary():
    pipeline = _patched_pipeline()
    pipeline._emit_execution_rejection_telemetry(symbol="BTC-USD", side="buy", reason="Kraken AddOrder rejected: EOrder:Insufficient margin")
...
def test_unknown_reject_still_uses_original_path():
    pipeline = _patched_pipeline(); request = SimpleNamespace(symbol="AXS-USD", side="buy")
    try: pipeline._on_order_rejected(request, "unknown exchange rejection")
