...
def _normalize_inst_id(symbol: Any) -> str:
    ...
    if "-" in raw:
        ...
        if quote == "USD" and base:
            return f"{base}-USDT"
        if quote in {"USDT", "USDC"} and base:
            return f"{base}-{quote}"
        return raw
...
def _looks_like_okx_order_class(cls: type) -> bool:
    """Patch only NIJA OKX broker/adapter classes, not low-level OKX SDK TradeAPI classes."""
    name = str(getattr(cls, "__name__", "")).lower()
    label = str(getattr(cls, "NAME", "") or getattr(cls, "name", "")).lower()
    compact = name.replace("_", "").replace("-", "")
    return (
        ("okx" in compact and ("broker" in compact or "adapter" in compact))
        or compact in {"okx", "okxbroker", "okxbrokeradapter"}
        or label in {"okx", "okxbroker", "okx broker"}
    )
...
def _payload_fields(payload: dict[str, Any]) -> tuple[Any, Any, Any]:
...
def _parse_order_call(args: tuple[Any, ...], kwargs: dict[str, Any]) -> tuple[Any, Any, Any, Optional[dict[str, Any]], tuple[Any, ...], dict[str, Any], str]:
    ...
    if args and isinstance(args[0], dict):
        payload = dict(args[0])
        symbol, side, quantity = _payload_fields(payload)
        return (
            symbol or kw.get("symbol") or kw.get("instId"),
            side or kw.get("side"),
            quantity or kw.get("quantity") or kw.get("qty") or kw.get("sz"),
            payload,
            tuple(args[1:]),
            kw,
            "payload",
    ...
    quantity = args[2] if len(args) >= 3 else (
        kw.get("quantity") or kw.get("qty") or kw.get("sz") or kw.get("size") or kw.get("size_usd")
        or kw.get("notional") or kw.get("notional_usd") or kw.get("order_notional") or kw.get("amount")
    )
    return symbol, side, quantity, None, tuple(args[3:]), kw, "expanded"


def _without_order_keys(kwargs: dict[str, Any]) -> dict[str, Any]:
    blocked = {
        "symbol", "instId", "instrument_id", "side", "action", "quantity", "qty", "sz", "size",
        "size_usd", "notional", "notional_usd", "order_notional", "amount",
    }
    ...
    *,
    ...
    if payload is not None:
        ...
        try:
            ...
            logger.warning("OKX_PAYLOAD_STYLE_SUBMIT_RETRY marker=%s method=%s instId=%s error=%s", _MARKER, method_name, symbol, exc)
    attempts = (
    ...
    for method_name in ("place_market_order", "execute_order", "place_order"):
        ...
        def _patched_order(self: Any, *args: Any, __fn: Callable[..., Any] = base, __method: str = method_name, **kwargs: Any) -> Any:
            ...
            if not inst_id or not order_side or order_qty <= 0:
                reason = f"OKX_CALLSHAPE_BLOCK missing_or_invalid_order_fields method={__method} symbol={symbol!r} side={side!r} quantity={quantity!r} shape={shape}"
                logger.critical("%s marker=%s", reason, _MARKER)
                print(f"[NIJA-PRINT] {reason} marker={_MARKER}", flush=True)
                return {"status": "error", "error": reason, "error_code": "OKX_CALLSHAPE_BLOCK", "symbol": inst_id, "side": order_side, "quantity": quantity}
            logger.critical(
                ...
                order_qty,
                shape,
                parsed_kwargs.get("order_type") or parsed_kwargs.get("ordType") or "market",
                parsed_kwargs.get("client_order_id") or parsed_kwargs.get("clOrdId") or "",
            )
            print(f"[NIJA-PRINT] FINAL_OKX_SUBMIT_CALL marker={_MARKER} method={__method} symbol={inst_id} side={order_side} quantity={order_qty}", flush=True)
            try:
                return _call_base(__fn, self, symbol=inst_id, side=order_side, quantity=order_qty, payload=payload, remaining=remaining, kwargs=parsed_kwargs, method_name=__method)
            except TypeError as exc:
                message = str(exc)
                if "symbol" in message and "quantity" in message:
                    ...
                    logger.critical("%s marker=%s symbol=%s side=%s", reason, _MARKER, inst_id, order_side)
                    return {"status": "error", "error": reason, "error_code": "OKX_CALLSHAPE_BLOCK", "symbol": inst_id, "side": order_side, "quantity": order_qty}
                raise
        ...
        _PATCHED_ORDER_CLASSES.add(f"{getattr(okx_cls, '__module__', '')}.{getattr(okx_cls, '__name__', '')}.{method_name}")
        logger.warning("OKX_FINAL_ORDER_METHOD_PATCHED marker=%s module=%s class=%s method=%s", _MARKER, module_name, getattr(okx_cls, "__name__", "<unknown>"), method_name)
        patched = True
    return patched
...
def _candidate_order_classes(module: ModuleType) -> list[type]:
    ...
    for obj in vars(module).values():
        if isinstance(obj, type) and obj not in found and _looks_like_okx_order_class(obj):
...
def _response_error(response: dict[str, Any]) -> str:
    ...
    return str(response.get("error") or response.get("message") or response.get("msg") or nested or response.get("code") or "unknown_okx_order_error")
...
def _patch_router_module(module: ModuleType) -> bool:
    ...
    def _patched_dispatch_direct_broker_market_order(broker: Any, *, symbol: str, side: str, size_usd: float, metadata: dict[str, Any]) -> tuple[float, float]:
        ...
        try:
            result = submit(inst_id, order_side, order_notional, size_type="quote")
        except TypeError as first_exc:
            try:
                result = submit(symbol=inst_id, side=order_side, quantity=order_notional, size_type="quote")
                ...
                result = submit(inst_id, order_side, order_notional)
        ...
        if status in {"error", "failed", "rejected", "canceled", "cancelled"} or (code and code != "0"):
            raise RuntimeError(f"OKX_ORDER_REJECTED code={code or result.get('error_code') or 'unknown'} message={_response_error(result)} raw={result!r}")
        fill_price = _f(result.get("filled_price") or result.get("average_filled_price") or result.get("average_fill_price") or result.get("avg_price") or result.get("price") or (metadata or {}).get("price_hint_usd"), 0.0)
        filled_usd = _f(result.get("filled_size_usd") or result.get("filled_value") or result.get("notional_usd") or result.get("size_usd"), order_notional)
        ...
        logger.critical("OKX_BROKER_ACK_RECEIVED marker=%s symbol=%s side=%s order_id=%s fill_price=%s filled_usd=%s", _MARKER, inst_id, order_side, order_id or "", fill_price, filled_usd)
...
def _interesting_module(name: str) -> bool:
    ...
    return lowered in {"bot.broker_manager", "broker_manager", "bot.broker_integration", "broker_integration", "bot.multi_account_broker_manager", "multi_account_broker_manager", "bot.multi_broker_execution_router", "multi_broker_execution_router"} or "okx" in lowered or "broker" in lowered
...
def _start_monitor() -> None:
    ...
    def _monitor() -> None:
        ...
        while time.time() < deadline:
            if _try_patch_loaded() and (_ROUTER_PATCHED or _PATCHED_ORDER_CLASSES):
                return
            time.sleep(0.25)
        logger.warning("OKX_FINAL_ORDER_SUBMISSION_BRIDGE_MONITOR_EXPIRED marker=%s router_patched=%s order_classes=%s", _MARKER, _ROUTER_PATCHED, sorted(_PATCHED_ORDER_CLASSES))
