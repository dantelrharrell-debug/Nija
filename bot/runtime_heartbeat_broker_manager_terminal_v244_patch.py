...
published; it is selection-only and grants no readiness or execution authority. v341
allows only the verified startup heartbeat to use NIJA's existing Coinbase micro-cap
minimum when cached balance proves that policy is applicable. v276 bridges only the
already-verified startup probe through the terminal lifecycle assertion that otherwise
calls the ordinary LIVE-only can_execute() path.
...
def _install_v341_heartbeat_microcap_proof() -> bool:
    """Install the verified-heartbeat-only Coinbase micro-cap bridge."""
    try:
        module = importlib.import_module("bot.runtime_heartbeat_microcap_proof_v341_patch")
        installer = getattr(module, "install", None) or getattr(module, "install_import_hook", None)
        ready = bool(callable(installer) and installer())
        if not ready:
            LOGGER.error("HEARTBEAT_BROKER_MANAGER_TERMINAL_V244_V341_INSTALL_FAILED marker=%s trading_fail_closed=true", MARKER)
        return ready
    except Exception as exc:
        LOGGER.error(
            "HEARTBEAT_BROKER_MANAGER_TERMINAL_V244_V341_INSTALL_ERROR marker=%s error=%s:%s trading_fail_closed=true",
            MARKER, type(exc).__name__, exc,
        )
        return False
...
def install() -> bool:
    try:
        ...
        v341_ready = _install_v341_heartbeat_microcap_proof()
        v276_ready = _install_v276_heartbeat_terminal_authority()
        ready = bool(
            upstream
            and methods_ready
            and context_handoff_ready
            and v255_ready
            and v273_ready
            and v274_ready
            and v341_ready
        ...
        ready, surfaces, context_handoff_ready, v255_ready, v273_ready, v274_ready, v341_ready, v276_ready = (
    ...
    if ready:
        LOGGER.critical(
            ...
            "heartbeat_microcap_proof_v341=true heartbeat_terminal_authority_v276=true "
...
__all__ = [
    ...
    "_install_v341_heartbeat_microcap_proof",
