...
# Always set at module level so the guard is reliable on subsequent imports.
_loaded = True
...
PLATFORM_FUNDING_RULES: Dict[str, PlatformFundingRules] = {
    ...
    'MICRO_PLATFORM': PlatformFundingRules(
        ...
        micro_platform_mode=True,  # Enable micro-platform optimizations
        max_trade_size_pct=40.0,  # Max 40% per trade — matches v71 required minimum (updated Apr 2026)
        min_trade_size_usd=1.0,  # Minimum $1 trades (micro-cap operational range, Coinbase only)
        max_positions=1,  # Single position only
