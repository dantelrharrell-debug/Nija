...
def _ok(payload: Any) -> bool:
    if isinstance(payload, dict):
        ...
        if payload.get("success") is True: return True
...
def _broker_label(broker: Any) -> str:
    try:
        bt = getattr(broker, "broker_type", None); raw = getattr(bt, "value", None) or bt
...
def _trigger(pos: dict[str, Any], price: float) -> tuple[bool, str, float]:
    ...
    if s in {"long", "buy"}:
        if stop > 0 and price <= stop: return True, "stop_loss", stop
        for name, target in tps:
            if target > 0 and price >= target: return True, name, target
    else:
        if stop > 0 and price >= stop: return True, "stop_loss", stop
        for name, target in tps:
            if target > 0 and price <= target: return True, name, target
...
def _scan_once(engine: Any) -> int:
    ...
    except Exception as exc: logger.warning("AUTO_EXIT_SCAN_OPEN_POSITIONS_FAILED err=%s", exc); return 0
    ...
    for pos in positions:
        ...
        if not _ok(order):
            ...
            continue
        fill = _f(_get(order, "filled_price", "average_fill_price", "avg_price", "price", default=p), p); fee = _f(_get(order, "fee", "commission", "fees", default=0.0)); oid = str(_get(order, "order_id", "id", "txid", "client_order_id", default="") or "")
        ...
        if pnl and pnl.get("success"):
            closed += 1; logger.critical("AUTO_EXIT_CLOSED symbol=%s position_id=%s net_pnl=%.8f pct=%+.4f%% reason=%s", symbol, pid, _f(pnl.get("net_profit")), _f(pnl.get("profit_pct")), reason)
        try: active.discard(symbol); active.discard(pid)
        except Exception: pass
    return closed
...
def _patch_engine(module: Any) -> bool:
    ...
    cls.scan_stop_loss_take_profit_once = _scan_once; cls.start_stop_loss_take_profit_monitor = _start_monitor; setattr(cls, _PATCHED, True); logger.warning("AUTO_EXIT_SL_TP_ENGINE_PATCHED"); return True


def _install_bridge(attr: str, envs: dict[str, str], modname: str, log_prefix: str) -> None:
    if getattr(builtins, attr, False): return
    for k, v in envs.items(): os.environ.setdefault(k, v)
    try: module = __import__(f"bot.{modname}", fromlist=["install_import_hook"])
    except Exception:
        try: module = __import__(modname, fromlist=["install_import_hook"])
        except Exception as exc: logger.warning("%s_BRIDGE_IMPORT_FAILED err=%s", log_prefix, exc); return
    try: module.install_import_hook(); setattr(builtins, attr, True); logger.warning("%s_BRIDGE_INSTALLED", log_prefix)
    except Exception as exc: logger.warning("%s_BRIDGE_INSTALL_FAILED err=%s", log_prefix, exc)


def _install_all_bridges() -> None:
    _install_bridge(_BRIDGES["trail_sl"], {"NIJA_TRAILING_STOP_ENABLED":"true", "NIJA_TRAILING_STOP_POLL_SECONDS":"5", "NIJA_TRAILING_STOP_PCT":"0.006", "NIJA_TRAILING_STOP_ACTIVATION_PCT":"0.003"}, "trailing_stop_loss_runtime_patch", "TRAILING_STOP")
    _install_bridge(_BRIDGES["breakeven"], {"NIJA_BREAKEVEN_STOP_ENABLED":"true", "NIJA_BREAKEVEN_STOP_POLL_SECONDS":"5", "NIJA_BREAKEVEN_PROFIT_THRESHOLD_PCT":"0.004", "NIJA_BREAKEVEN_STOP_OFFSET_PCT":"0.0002"}, "breakeven_stop_loss_runtime_patch", "BREAKEVEN_STOP")
    ...
    _install_bridge(_BRIDGES["trail_tp"], {"NIJA_TRAILING_TP_ENABLED":"true", "NIJA_TRAILING_TP_POLL_SECONDS":"5", "NIJA_TRAILING_TP_ACTIVATION_PCT":"0.008", "NIJA_TRAILING_TP_CALLBACK_PCT":"0.003"}, "trailing_take_profit_runtime_patch", "TRAILING_TP")
    _install_bridge(_BRIDGES["combined"], {"NIJA_COMBINED_TRAILING_TP_SL_ENABLED":"true", "NIJA_COMBINED_TRAILING_POLL_SECONDS":"5", "NIJA_COMBINED_TRAILING_SL_ACTIVATION_PCT":"0.003", "NIJA_COMBINED_TRAILING_SL_DISTANCE_PCT":"0.006", "NIJA_COMBINED_TRAILING_TP_ACTIVATION_PCT":"0.008", "NIJA_COMBINED_TRAILING_TP_CALLBACK_PCT":"0.003"}, "combined_trailing_tp_sl_runtime_patch", "COMBINED_TRAILING")
...
def install_import_hook() -> None:
    ...
    if getattr(builtins, "_NIJA_AUTO_EXIT_SL_TP_IMPORT_HOOK", False): return
    ...
    def hook(name: str, globals=None, locals=None, fromlist=(), level: int = 0):
        ...
        except Exception as exc: logger.warning("AUTO_EXIT_SL_TP_PATCH_FAILED module=%s err=%s", name, exc)
        return mod
    builtins.__import__ = hook; setattr(builtins, "_NIJA_AUTO_EXIT_SL_TP_IMPORT_HOOK", True); logger.warning("AUTO_EXIT_SL_TP_IMPORT_HOOK_INSTALLED")
