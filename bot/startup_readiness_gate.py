...
class StartupReadinessGate:
    ...
    def signal_ready(self, name: str) -> None:
        ...
        with self._cond:
            ...
            # in systems that assume exact signal names.
            if name not in self._required and self._required:
                _ci_matches = [n for n in self._required if n.lower() == name.lower()]
                if _ci_matches:
                    logger.critical(
                        "❌ BARRIER SIGNAL NAME MISMATCH: received=%s expected=%s",
                        name,
                        ",".join(sorted(_ci_matches)),
                    )
