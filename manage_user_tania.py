#!/usr/bin/env python3
"""
User Management Interface for Tania Gilbert

This script provides a simple interface to manage Tania Gilbert's trading account.
You can enable/disable trading and check status without affecting other users.

Usage:
    python manage_user_tania.py status        # Check current status
    python manage_user_tania.py enable        # Enable trading
    python manage_user_tania.py disable       # Disable trading
    python manage_user_tania.py info          # Show detailed info
"""
...
try:
    from init_user_tania import load_user_database
    load_user_database()
except Exception as e:
    print(f"⚠️  Warning: Could not load user database: {e}")
    print("   Run 'python init_user_tania.py' first to initialize user.")


def print_status(user_id="tania_gilbert"):
    ...
    if not user:
        ...
        print("Run init_user_tania.py first to create the user.")
        return False
    ...
    print("\n" + "="*60)
    print(f"USER: Tania Gilbert ({user_id})")
    print("="*60)
...
def show_detailed_info(user_id="tania_gilbert"):
    ...
    print("\n" + "="*60)
    print(f"DETAILED INFO: Tania Gilbert")
    print("="*60)
...
def enable_trading(user_id="tania_gilbert"):
...
def disable_trading(user_id="tania_gilbert"):
...
def print_usage():
    ...
    print("\nUsage: python manage_user_tania.py <command>")
    print("\nCommands:")
    ...
    print("\nExamples:")
    print("  python manage_user_tania.py status")
    print("  python manage_user_tania.py enable")
    print("  python manage_user_tania.py disable")
    print("  python manage_user_tania.py info")
    print()
