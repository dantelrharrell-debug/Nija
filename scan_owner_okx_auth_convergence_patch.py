        ...
        while waited < wait_timeout:
            ...
            if current_age > stall_threshold:
                logger.critical(
                    ...
                    MARKER, key, thread_id, state.owner_thread_id,
                    current_age, stall_threshold,
                )
                with _SCAN_STATES_GUARD:
                    _SCAN_STATES[key] = _ScanState()
                return _empty_result("scan_stall_reset")

        logger.error(
            "SCAN_OWNER_TIMEOUT marker=%s identity=%s owner_thread=%s age_s=%.2f waited_s=%.2f",
            MARKER, key, state.owner_thread_id,
            max(0.0, time.monotonic() - state.started_at), waited,
        )
