...
Execution now enforces positive expectancy and edge quality before any new entry:

$$
E = p(win) \times AvgWin - (1-p(win)) \times AvgLoss - Costs
$$

Add or tune these variables in `.env`:

```bash
EXPECTED_WIN_RATE_PCT=50
MIN_EXPECTANCY_THRESHOLD_PCT=0.0
MIN_EDGE_SCORE_THRESHOLD=0.0
MARKET_QUALITY_THRESHOLD=0.55
MIN_TP_PCT=0.012
MAX_SL_PCT=0.010
MAX_KELLY_FRACTION=0.25
PAIR_EXPECTANCY_WINDOW=100
PAIR_EXPECTANCY_MIN_TRADES=50
EXPECTANCY_DISABLE_COOLDOWN_MINUTES=240
EXPECTANCY_FORCE_ENABLE_BUCKETS=
```

Behavior summary:
- Entries are rejected when expected value or edge score is below threshold.
- Sizing is clamped by a Kelly-lite fraction before order routing.
- Symbol+regime buckets can be auto-disabled when rolling realized expectancy remains negative.
- Disabled buckets auto re-enable after cooldown, or immediately via `EXPECTANCY_FORCE_ENABLE_BUCKETS`.
- Runtime status is available in the execution engine via `get_expectancy_kill_switch_status()` and `log_expectancy_kill_switch_status()`.

To keep Alpaca disabled while focusing on crypto brokers:
...
2. Startup reconciliation must complete before LIVE activation when `NIJA_REQUIRE_STARTUP_RECONCILIATION=true` (recommended for production).
3. In live mode, distributed single-writer lock + nonce lease ownership are required.
4. Nonce lease stability checks are enforced when `LIVE_CAPITAL_VERIFIED=true` or `NIJA_REQUIRE_NONCE_LEASE_STABILITY=true` (default 30s window via `NIJA_NONCE_LEASE_STABILITY_S`).
...
HEARTBEAT_TRADE_SIZE=10.00  # Requested trade size (cap will clamp to $5.00)
...
**What Happens:**
```
❤️  HEARTBEAT TRADE ENABLED: request=$10.00 cap=$5.00 every 600s
...
