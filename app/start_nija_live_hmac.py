...
async def verify_hmac():
    ...
    if not accounts:
        raise Exception("❌ HMAC verification failed: no accounts fetched.")
    print("✅ HMAC verification complete. Accounts:")
    for acct in accounts:
        print(f"{acct['name']} ({acct['currency']}): {acct['balance']['available']}")
    return accounts

# ---------------- Main Loop ----------------
async def main_loop():
    while True:
        accounts = await get_accounts()  # <-- refresh accounts each cycle
        if not accounts:
