    ...
    async with SEM:
        async with aiohttp.ClientSession() as session:
            async with session.request(method, url, headers=headers, data=body if body else None) as resp:
                if resp.status >= 400:
                    logger.warning(f"HTTP {resp.status} error on {path}")
                    return None
...
async def execute_order(account_id, symbol, side, size):
    if size <= 0:
        logger.warning(f"[{account_id}] Trade size zero, skipping {symbol}")
        return None
...
async def check_trailing(account_id, symbol, prices):
    async with TRADE_STATE_LOCK:
        state = TRADE_STATE.get(account_id, {}).get(symbol)
        if not state:
            return
        side, entry, ttp, tsl, size = state["side"], state["entry"], state["ttp"], state["tsl"], state["size"]
        current_price = prices[-1]

        if side == "buy":
            if current_price > ttp: state["ttp"] = current_price * 0.99
            if current_price - entry > 0: state["tsl"] = max(tsl, current_price * 0.98)
            ...
            if current_price < ttp: state["ttp"] = current_price * 1.01
            if entry - current_price > 0: state["tsl"] = min(tsl, current_price * 1.02)
            if current_price > tsl or current_price > ttp:
