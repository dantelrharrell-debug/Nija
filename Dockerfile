...
RUN sh -eux -c '\
      if [ -d /src/vendor/coinbase_advanced_py ]; then \
        ...
        echo "Vendor not present in build context; skipping clone"; \
      fi'
...
ENV PYTHONUNBUFFERED=1 PIP_NO_CACHE_DIR=1 NIJA_ENV=production
WORKDIR /usr/src/app
...
FROM builder AS dev
ENV PYTHONUNBUFFERED=1 PIP_NO_CACHE_DIR=1 NIJA_ENV=development
WORKDIR /usr/src/app
COPY . /usr/src/app
RUN python3 -m pip install --upgrade pip setuptools wheel \
 && if [ -f /usr/src/app/requirements.txt ]; then python3 -m pip install -r /usr/src/app/requirements.txt; fi \
