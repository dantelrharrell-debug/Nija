...
# Build arguments for Git/deployment metadata. Railway exposes predefined
# variables to Docker builds when matching ARG declarations are present.
ARG GIT_BRANCH=""
ARG GIT_COMMIT=""
...
# Railway/git fallback generated by inject_git_metadata.sh.
...
# Default command: resolve deployment provenance and guards before start.sh.
